...
```{r}
bac.con0 = transform_sample_counts(bacteria.con, 
                                                      function(x) x / sum(x))
```
...
```{r}
glom.bac.con1 <- tax_glom(bac.con0,taxrank = 'Phylum')
data_glom.bac.con2 <- psmelt(glom.bac.con1)
data_glom.bac.con2$Phylum <- as.character(data_glom.bac.con2$Phylum)
```

```{r}
unique(data_glom.bac.con2$Phylum)
```


```{r}
Acidobacteria.con <- subset(data_glom.bac.con2, Phylum == "Acidobacteria")
Actinobacteria.con <- subset(data_glom.bac.con2, Phylum == "Actinobacteria")
Bacteroidetes.con <- subset(data_glom.bac.con2, Phylum == "Bacteroidetes")
Chloroflexi.con <- subset(data_glom.bac.con2, Phylum == "Chloroflexi")
Planctomycetes.con <- subset(data_glom.bac.con2, Phylum == "Planctomycetes")
Proteobacteria.con<- subset(data_glom.bac.con2, Phylum == "Proteobacteria")
```
