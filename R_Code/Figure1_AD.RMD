...
fun.june.con.plot <- ggplot(data_glom.fun.june2.top6,aes(Grass,Abundance,color=Class)) + geom_boxplot() +
  scale_color_manual(values = c("indianred1", "lightsalmon","khaki1","lightgreen", "lightblue1", "royalblue3")) + theme_bw(base_size = 20) + 
  labs(y= "Relative Abundance", x = "") + 
...
fun.nov.con.plot <- ggplot(data_glom.fun.nov2.top6,aes(Grass,Abundance,color=Class)) + geom_boxplot() +
  scale_color_manual(values = c("indianred1", "lightsalmon","khaki1","grey26", "lightblue1", "royalblue3")) + theme_bw(base_size = 20) + labs(y= "", x = "") + 
...
```{r}
Pre_Post_Fun.plot = ggarrange(fun.june.con.plot, fun.nov.con.plot, ncol=2, common.legend = TRUE, legend = "right")#, labels = c("C", "D"))
Pre_Post_Fun.plot
```
...
```{r warning=FALSE}
pdf("../Figures/Figure_1_Pre_Post.pdf", width = 12, height = 10 )
figure_1
dev.off()
```
